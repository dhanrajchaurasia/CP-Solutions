...
char arr[1001][1001];
bool chk(int i, int j, int n, bool a, bool b)
{
    int x = 0, y = 2, cnt = 0;
    while (i + x * a < n and j + x * b < n)
    {
        if(arr[i + x * a][j + x * b] == '.')
        {
            if (y)
...
void solve()
{
    ...
    rep(i, 0, n)
    {
        rep(j, 0, n)
        {
            if(chk(i, j, n, true, true) or chk(i, j, n, true, false) or chk(i, j, n, false, true))
            {
                cout << "Yes" << endl;
                return;
            }
