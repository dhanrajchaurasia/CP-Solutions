...
int knapsack(int n, int w, vi &W, vi &V, vvi &dp)
{
    if (n == 0)
        return 0;
    if (dp[w][n - 1] != -1)
    ...
    int res = dp[w][n];
    if (w >= W[n - 1])
        res = max(res, knapsack(n - 1, w - W[n - 1], W, V, dp) + V[n - 1]);
    res = max(res, knapsack(n - 1, w, W, V, dp));
    return dp[w][n - 1] = res;
}
...
void solve()
{
    int n, w;
    cin >> n >> w;
    vec<int> dp(w + 1, vi(n + 1, -1));
    vi W(n), V(n);
    rep(i, 0, n)
        cin >> W[i] >> V[i];
    cout << knapsack(n, w, W, V, dp) << endl;
}  
