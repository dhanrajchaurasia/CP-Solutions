...
public:
    void insert(double m, double c){
        while (!hull.empty()){
            line k = hull.back();
            if (k.c + k.m * k.x < c + m * k.x){
                if (k.m - m) hull.push_back({(c - k.c) / (double)(k.m - m), m, c});
                return;
            }
            hull.pop_back();
        }
        hull.push_back({LLONG_MIN, m, c});
    }
    double query(double x){
        line k = *--upper_bound(all(hull), x, [](double a, line b){return a < b.x;});
        return k.m * x + k.c;
    }
};
void solve()
{
    ...
    rep(i, 0, n){
        ...
        cin >> x;
        if (i)
            ans = x * x + hull.query(x) + k;
        hull.insert(-2 * x, ans + x * x);
    }
    cout << ans << endl;
}  
