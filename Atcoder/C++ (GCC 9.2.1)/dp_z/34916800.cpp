...
public:
    LiChaoTree(int _n){
        n = _n;
        tree.resize(4 * n + 1, {0, inf});
    }
    T f(vector<T> line, T x){
        return line[0] * x + line[1];
    }
    T op(T x, T y){
        return min(x, y);
    }
    void insert(int i, int l, int r, vector<T> line){
        int m = (l + r) >> 1;
        bool left = f(line, l) < f(tree[i], l);
        bool mid = f(line, m) < f(tree[i], m);
        if (mid) swap(tree[i], line);
        if (r - l == 1) return;
        if (left != mid) insert(2 * i, l, m, line);
        else insert(2 * i + 1, m, r, line);
    }
    T query(T x, int i, int l, int r){
        int m = (l + r) >> 1;
        T cur = f(tree[i], x);
        if (r - l == 1) return cur;
        if (x < m) return op(cur, query(x, 2 * i, l, m));
        return op(cur, query(x, 2 * i + 1, m, r));
    }
};
void solve()
{
    ...
    vi dp(n);
    /*
        dpj = min(dpj, dpi + (hj - hi)^2)
        dpj = dpi + hj * hj - 2 * hi * hj + hi * hi = -2 * hi * hj + (dpi + hi * hi)
        y = dpj, m = -2 * hi, x = hj, c = dpi + hi * hi
    */
    int N = 1e6 + 1;
    LiChaoTree<int> f(N);
    rep(i, 0, n)
    {
        ...
        cin >> x;
        if (i)
            dp[i] = f.query(x, 1, 1, N) + c + x * x;
        f.insert(1, 1, N, {-2 * x, dp[i] + x * x});
    }
    cout << dp[n - 1] << endl;
}  
