...
private:
	...
	public:
        ...
        T val (T x) {return slope * x + yIntercept;}
        T intersect(Line y) {return (y.yIntercept - yIntercept + slope - y.slope - 1) / (slope - y.slope);}
    };
public:
    deque<pair<Line, T>> dq;
    void insert(T slope, T yIntercept){
        Line newLine(slope, yIntercept);
        while (size(dq) > 1 and dq.back().second >= dq.back().first.intersect(newLine)) dq.pop_back();
    ...
    T query2(T x){
        auto qry = *lower_bound(dq.rbegin(), dq.rend(), make_pair(Line(0, 0), x), [&](const pair<Line, T> &a, pair<Line, T> &b) {return a.second > b.second;});
...
void solve()
{
    ...
    cin >> n >> c;
    vi dp(n);
    ConvexHullTrick<int> chull;
    rep(i, 0, n)
    {
        int x;
        cin >> x;
        if (i)
            dp[i] = chull.query(x) + c + x * x;
        chull.insert(-2 * x, dp[i] + x * x);
    }
    cout << dp[n - 1] << endl;
}  
