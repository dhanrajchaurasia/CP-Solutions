...
public:
    ...
    void build(int x, int l, int r)
    {
    	if (r < l)
        	return;
        if (l == r)
            return seg_tree[x].pb(arr[l]);
        int mid = (l + r) / 2LL;
        build(2 * x, l, mid);
        build(2 * x + 1, mid + 1, r);
        merge(seg_tree[x], seg_tree[2 * x], seg_tree[2 * x + 1]);
    }
    ...
    int get(int val, vector<int> &arr)
    {
        auto idx1 = ub(all(arr), val);
        auto idx2 = lb(all(arr), val);
        return idx1 - idx2;
    }
    int queryIs(int x, int L, int R, int l, int r, int val)
    {
        if (R < l or L > r)
            return 0;
        if (l <= L and R <= r)
            return get(val, seg_tree[x]);
        int mid = (L + R) / 2LL;
        return queryIs(2 * x, L, mid, l, r, val) + queryIs(2 * x + 1, mid + 1, R, l, r, val);
    }
    int query(int l, int r, int val)
    {
        return queryIs(1, 1, n, l, r, val);
    }
};
void solve()
{
