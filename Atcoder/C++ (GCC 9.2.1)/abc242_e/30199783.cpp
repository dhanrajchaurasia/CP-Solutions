...
int dp[maxx][2];
int recur(string &s, int i, int n, int tight)
{
    if (i == n)
        return 1;
    if (dp[i][tight] != -1)
        return dp[i][tight];
    int &ans = dp[i][tight] = 0;
    int x = tight ? s[i] - 'A' : 25;
    rep(j, 0, x + 1)
        ans = mod_add(ans, recur(s, i + 1, n, tight & (j == x)), mod2);
    return ans;
}
void solve()
{
    ...
    rep(i, n / 2, n)
        k[i] = s[n - i - 1];
    n = (n + 1) / 2LL;
    cout << mod_sub(recur(k, 0, n, 1), (k > s), mod2) << endl;
}  
