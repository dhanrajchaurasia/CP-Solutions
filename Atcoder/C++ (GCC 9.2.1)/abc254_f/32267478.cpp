    ...
    public:
        ...
        SparseTable(vector<T> &arr)
        {
            ...
            for(int i = 2; i <= n; i++)
                log_2[i] = log_2[i / 2] + 1;
            int k = log_2[n];
            Table.resize(n, vector<T>(k + 1, 0));
            for (int i = 1; i < n; i++)
                Table[i][0] = abs(arr[i] - arr[i - 1]);
            for(int i = 1; i <= k; i++)
                for(int j = 0; j <= n - (1LL << i); j++)
                    Table[j][i] = op(Table[j][i - 1], Table[j + (1LL << (i - 1))][i - 1]);
        }
        int query(int l, int r)
        {
            int k = log_2[r - l + 1];
            return op(Table[l - 1][k], Table[r - (1LL << k)][k]);
        }
};
...
void solve()
{
    ...
    vi a(n), b(n);
    cin >> a >> b;
    SparseTable<int> stA(a), stB(b);
    // A is in rows and B is in cols.
    rep(i, 0, q)
    {
        int h1, h2, w1, w2;
        cin >> h1 >> h2 >> w1 >> w2;
        int ans1 = (h1 < h2 ? stA.query(h1 + 1, h2) : 0);
        int ans2 = (w1 < w2 ? stB.query(w1 + 1, w2) : 0);
        cout << gcd(gcd(ans1, ans2), a[h1 - 1] + b[w1 - 1]) << endl;
    }
}  
