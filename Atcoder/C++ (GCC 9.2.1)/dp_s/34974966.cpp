...
int recur(string &s, int i, int rem, int tight, int zro, int d){
    if (i == size(s))
        return rem == 0 and zro == 0;
    int &ans = dp[i][rem][tight][zro];
    if (ans != -1)
        return ans;
    ans = 0;
    int x = (tight ? s[i] - '0' : 9);
    if (zro)
        ans = (ans + recur(s, i + 1, rem, 0, 1, d)) % mod;
    rep(j, 0, x + 1)
    {
        if (zro & (j == 0))
            continue;
        ans = (ans + recur(s, i + 1, (rem + j) % d, tight & (j == x), zro & (j == 0), d)) % mod;
    }
    return ans;
}
