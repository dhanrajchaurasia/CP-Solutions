...
template <typename T1, typename T2> inline void amax(T1 &x, T2 y) {if (y > x) x = y;}
template <typename T1, typename T2> inline void amin(T1 &x, T2 y) {if (y < x) x = y;}
...
void solve()
{
	int n;
	cin >> n;
	vec<double> dp(2, vector<double>(n + 1));
	dp[0][0] = 1;
	rep(i, 1, n + 1)
	{
		double p_head;
		cin >> p_head;
		rep(j, 0, i + 1)
			dp[i % 2][j] = dp[(i - 1) % 2][j] * (1 - p_head) + (j != 0 ? dp[(i - 1) % 2][j - 1] * p_head : 0);
	}
	double ans = 0;
	rep(heads, 0, n + 1)
	{
