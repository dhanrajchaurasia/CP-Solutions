...
int dp[2001][2001][2];
int n, m, k, a, b, x;
int dfs(int a, int moves, bool freq)
{
    if (moves == 0)
        return a == b and !freq;
    if (dp[a][moves][freq] != -1)
        return dp[a][moves][freq];
    int &ans = dp[a][moves][freq] = 0;
    for (auto &node : adj[a])
    {
        if (node == x)
            ans = mod_add(ans, dfs(node, moves - 1, freq ^ 1), mod2);
        else
            ans = mod_add(ans, dfs(node, moves - 1, freq), mod2);
    }
    return ans;
}
void solve()
