...
const int N = 22;
mii dp[N][N];
void dfs1(int i, int j, int k, int cnt, vec<int> &a){
    k ^= a[i][j];
    if (cnt == size(a) - 1){
        dp[i][j][k]++;
        return;
    ...
    if (i + 1 < size(a))
        dfs1(i + 1, j, k, cnt + 1, a);
    if (j + 1 < size(a))
        dfs1(i, j + 1, k, cnt + 1, a);
}
int ans = 0;
void dfs2(int i, int j, int k, int cnt, vec<int> &a){
    if (cnt == size(a) - 1){
        ans += dp[i][j][k];
        return;
    }
    k ^= a[i][j];
    if (i - 1 >= 0)
        dfs2(i - 1, j, k, cnt + 1, a);
    if (j - 1 >= 0)
        dfs2(i, j - 1, k, cnt + 1, a);
}
