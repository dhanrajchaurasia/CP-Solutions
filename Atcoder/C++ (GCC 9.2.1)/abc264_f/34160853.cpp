        ...
        rep(j, 0, m)
        {
            rep(a, 0, 2)
            {
                rep(b, 0, 2)
                {
                    ...
                    {
                        if ((arr[i + 1][j] == k) ^ b)
                            amin(dp[i + 1][j][0][b], dp[i][j][a][b]);
                        else
                            amin(dp[i + 1][j][1][b], dp[i][j][a][b] + r[i + 1]);
                    }
                    if (j < m - 1)
                    {
                        if ((arr[i][j + 1] == k) ^ a)
                            amin(dp[i][j + 1][a][0], dp[i][j][a][b]);
                        else
                            amin(dp[i][j + 1][a][1], dp[i][j][a][b] + c[j + 1]);
                    }
                }
            }
        }
