...
void solve()
{
    int n, m, k;
    cin >> n >> m >> k;
    int dp[n][m + 1], pre[m + 1];
    mem(dp, 0);
    mem(pre, 0);
    rep(i, 1, m + 1)
    {
        dp[0][i] = 1;
        pre[i] = mod_add(pre[i - 1], dp[0][i], MOD[0]);
    }
    rep(i, 1, n)
    {
        rep(j, 1, m + 1)
        {
            int mx = min(m + 1, j + k);
            int mn = max(0LL, j - k);
            int x = mod_sub(pre[m], pre[mx - 1], MOD[0]);
            if (k == 0)
                dp[i][j] = mod_add(pre[mn - 1], x, MOD[0]);
            else
                dp[i][j] = mod_add(pre[mn], x, MOD[0]);
        }
        mem(pre, 0);
        rep(j, 1, m + 1)
            pre[j] = mod_add(pre[j - 1], dp[i][j], MOD[0]);
    }
    cout << pre[m] << endl;
}  
