...
int ceili(int x, int y) {return (x >= 0 ? (x + y - 1) / y : x / y);}
...
{
    /*
        dpi = 1 + [dpi + dpi+1 + dpi+2..... dpai] / (ai + 1)
        dpi - dpi / (ai + 1) = dpi * ai / (1 + ai) = 1 + [dpi+1...dpai] / (ai + 1)
        dpi = (ai + 1) / ai + [dpi+1... dpai] / ai
    */
    int n;
    cin >> n;
    vi v(n - 1), dp(n + 2);
    ...
    {
        int x = v[i - 1];
        int ans = mod_div(x + 1, x, mod2);
        int y = mod_sub(dp[i + 1], dp[i + 1 + x], mod2);
        ans = mod_add(ans, mod_div(y, x, mod2), mod2);
        dp[i] = mod_add(dp[i + 1], ans, mod2);
    }
    cout << mod_sub(dp[1], dp[2], mod2) << endl;
}  
