...
struct Node* buildTree(vi &in, vi &pre, int inStrt, int inEnd, mii& mp)
{
...
struct Node* buldTreeWrap(vi &in, vi &pre, int len)
{
    mii mp;
    for (int i = 0; i < len; i++)
        mp[in[i]] = i;
...
bool checkInorderPreorderUtil(int inStart, int inEnd, int& preIndex, vi &preorder, mii& inorderIndexMap)
{
    if (inStart > inEnd)
        return true;
...
bool checkInorderPreorder(vi &pre, vi &in, int n)
{
    mii inorderIndexMap; 
    for (int i = 0; i < n; i++)
        inorderIndexMap[in[i]] = i; 
    int preIndex = 0;
    return(checkInorderPreorderUtil(0, n - 1, preIndex, pre, inorderIndexMap));
}
pii ans[maxn];
void dfs(struct Node *root)
{
    ...
    int x = root->data;
    if (root->left)
        ans[x].ff = root->left->data;
    if (root->right)
        ans[x].ss = root->right->data;
    dfs(root->left);
    dfs(root->right);
}
void solve()
{
    ...
    vi a(n), b(n);
    cin >> a >> b;
    if (a[0] != 1 or !checkInorderPreorder(a, b, n))
    {
        cout << -1 << endl;
        return;
    }
    struct Node *root = buldTreeWrap(b, a, n);
    dfs(root);
    rep(i, 1, n + 1)
