...
void solve()
{
    ...
    rep(i, 0, n)
        rep(j, 0, n - 2)
            if (a[j] % 2 != a[j + 2] % 2 and a[j] % 2 != j % 2) // even should be on even pos.
                swap(a[j], a[j + 2]), ans.pb({'B', j + 1});
    rep(i, 0, n - 1)
        if (a[i] != a[i + 1] and a[i] % 2 == i % 2) // even should be on odd pos
            swap(a[i], a[i + 1]), ans.pb({'A', i + 1});
    rep(i, 0, n)
        rep(j, 0, n - 2)
            if (a[j] > a[j + 2])
                swap(a[j], a[j + 2]), ans.pb({'B', j + 1});
