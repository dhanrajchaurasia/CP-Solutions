...
void solve()
{
    ...
    rep(i, 0, n)
    {
        ...
        if (c == 'L')
        {
            if (left.find(v[i].ss) == left.end())
                left[v[i].ss] = v[i].ff;
            else
                left[v[i].ss] = max(left[v[i].ss], v[i].ff);
        }
        else
        {
            if (right.find(v[i].ss) == right.end())
                right[v[i].ss] = v[i].ff;
            else
                right[v[i].ss] = min(right[v[i].ss], v[i].ff);
        }
    }
    for (auto &i : left)
    {
        if (right.find(i.ff) == right.end())
            continue;
        if (right[i.ff] <= left[i.ff])
        {
            cout << "Yes" << endl;
            return;
        }
