...
void solve()
{
    ...
    rep(i, 0, q)
    {
        if (qry[i][0] == 1)
        {
            col.update(qry[i][1], qry[i][3]);
            col.update(qry[i][2] + 1, -qry[i][3]);
        }
        else if (qry[i][0] == 2)
        {
            for (auto &e : arr[i])
            {
                ans[e] -= col.query(1, qry[e][2]);
                ans[e] += qry[i][2];
            }
        }
        else
            cout << ans[i] + col.query(1, qry[i][2]) << endl;
    }
}  
