...
void solve()
{
    ...
    rep(i, 4, 8)
    {
        int x = x1 - dx[i], y = y1 + 2 * dy[i];
        int xx = x1 + 2 * dy[i], yy = y1 - dx[i];   
        int x_ = x2 - dx[i], y_ = y2 + 2 * dy[i];
        int xx_ = x2 + 2 * dy[i], yy_ = y2 - dx[i];        
        if (dist(x1, x, y1, y) == dist(x2, x, y2, y))
        {
            cout << "Yes" << endl;
            return;
        }
        if (dist(x1, x_, y1, y_) == dist(x2, x_, y2, y_))
        {
            cout << "Yes" << endl;
            return;
        }
        if (dist(x1, xx, y1, yy) == dist(x2, xx, y2, yy))
        {
            cout << "Yes" << endl;
            return;
        }
        if (dist(x1, xx_, y1, yy_) == dist(x2, xx_, y2, yy_))
        {
            cout << "Yes" << endl;
            return;
        }
